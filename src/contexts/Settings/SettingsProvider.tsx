...
function SettingsProvider({children}: { children: ReactNode }) {
    ...
    const [config, setConfig] = useState<Config>(() => {
        ...
        if (savedBaudRate) invoke("set_baud_rate", {baudRate: +savedBaudRate});
        if (savedDataBits) invoke("set_data_bits", {dataBits: savedDataBits});
        if (savedStopBits) invoke("set_stop_bits", {stopBits: savedStopBits});
        if (savedParity) invoke("set_parity", {parity: savedParity});
